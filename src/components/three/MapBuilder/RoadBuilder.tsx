...
const RoadBuilder: React.FC = () => {
  ...
  useEffect(() => {
    // Clear preview when not in straight road mode
  ...
  const createCurvedRoad = (_point: THREE.Vector3) => {
    // TODO: Implement curved road creation
    console.log('Curved road creation not implemented yet');
    return null;
  };

  const createCircularRoad = (_point: THREE.Vector3) => {
    ...
    if (mousePosition) {
      ...
      switch (activeSubMenu) {
        case 'map-menu-1': // Straight Road
          newEdge = createStraightRoad(mousePosition);
          break;
        case 'map-menu-2': // Curved Road
          newEdge = createCurvedRoad(mousePosition);
          break;
        case 'map-menu-3': // Circular Road
          newEdge = createCircularRoad(mousePosition);
          break;
        default:
      ...
      if (newEdge) {
        addEdge(newEdge);
        // Clear preview after creating actual road
        clearPreviewEdge();
        clearPreviewNodes();
        previewInitializedRef.current = false;
      }
    }
  }, [activeMainMenu, activeSubMenu, getMousePosition3D, createStraightRoad, createCurvedRoad, createCircularRoad, addEdge, clearPreviewEdge, clearPreviewNodes]);

  // Only attach onPointerMove when straight road is selected
